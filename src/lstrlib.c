...
static int str_gsub (lua_State *L) {
  ...
  //ctx = lua_getallocf(L, &alloc_ud)(alloc_ud, NULL, 0, sizeof(struct gsub_state));
  if (lua_isnone(L, 4)) lua_pushnil(L);
  ctx = lua_newuserdata(L, sizeof(struct gsub_state));
  ctx->src = src;
  ctx->srcl = srcl; 
  ctx->p = p;
  ctx->lp = lp;
...
static const char *checklstring_nil(lua_State *L, int narg, size_t *len) {
  if (lua_isnil(L, narg)) {
    if (len != NULL) *len = 3;
    return "nil";
  }
  const char *s = luaL_tolstring(L, narg, len);
  if (!s) luaL_error(L, "bad argument #%d (expected string, got %s)", narg, lua_typename(L, lua_type(L, narg)));
  return s;
}
  ...
  while (strfrmt < strfrmt_end) {
    if (*strfrmt != L_ESC)
      ...
      switch (*strfrmt++) {
        ...
        case 's': {
          size_t l;
          const char *s = checklstring_nil(L, arg, &l);
